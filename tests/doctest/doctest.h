...
//
// Ported to C++ 98 by Aleksandr Gubanov 2019
//
...
#define DOCTEST_VERSION_STR "1.3.6"
...
#elif defined(__attribute__) // MSVC
...
#ifndef DOCTEST_NO_CPP11_COMPAT
...
#define DOCTEST_ASSERT_IN_TESTS(decomp)                                                            \
    ...
    public:
        ...
        ~ContextScope() DOCTEST_OVERRIDE { destroy(); }
    };
    ...
    struct ContextScopeLambda
    {
        ...
        ContextScopeLambda(const char *file, int line, std::string expression_):
            mb_name(file, line, doctest::assertType::is_warn),
            expr(expression_)
        {}
...
#define DOCTEST_TEST_CASE_TEMPLATE_INVOKE(id, ...)                                                 \
    DOCTEST_TEST_CASE_TEMPLATE_INSTANTIATE_IMPL(id, DOCTEST_ANONYMOUS(_DOCTEST_ANON_TMP_), doctest::detail::strip_types<__VA_ARGS__>::type) \
    typedef int DOCTEST_ANONYMOUS(_DOCTEST_ANON_FOR_SEMICOLON_)
...
#define DOCTEST_REGISTER_EXCEPTION_TRANSLATOR_IMPL(translatorName, signature)                      \
    ...
    DOCTEST_GLOBAL_NO_WARNINGS(DOCTEST_ANONYMOUS(_DOCTEST_ANON_TRANSLATOR1_)) =                     \
            doctest::registerExceptionTranslator(translatorName);                                  \
    DOCTEST_GLOBAL_NO_WARNINGS_END()                                                               \
    doctest::String translatorName(signature)

#define DOCTEST_REGISTER_EXCEPTION_TRANSLATOR(signature)                                           \
    DOCTEST_REGISTER_EXCEPTION_TRANSLATOR_IMPL(DOCTEST_ANONYMOUS(_DOCTEST_ANON_TRANSLATOR2_),       \
                                               signature)
...
#define DOCTEST_INFO_IMPL(lambda_name, mb_name, s_name, expression)                                \
    ...
    DOCTEST_MSVC_SUPPRESS_WARNING_POP                                                              \
    doctest::detail::ContextScope<doctest::detail::ContextScopeLambda> DOCTEST_ANONYMOUS(_DOCTEST_CAPTURE1_) = doctest::detail::MakeContextScope(lambda_name)
#endif // DOCTEST_NO_CPP11_COMPAT
...
#define DOCTEST_ASSERT_THROWS(expr, assert_type)                                                   \
    do {                                                                                           \
        if(!doctest::getContextOptions()->no_throw) {                                   \
            doctest::detail::ResultBuilder _DOCTEST_RB(doctest::assertType::assert_type,   \
...
#define DOCTEST_ASSERT_THROWS_AS_MESSAGE(expr, as, assert_type, message)                                            \
    do {                                                                                           \
        if(!doctest::getContextOptions()->no_throw) {                                   \
            doctest::detail::ResultBuilder _DOCTEST_RB(                                            \
                    doctest::assertType::assert_type, __FILE__, __LINE__, #expr,           \
...
#define DOCTEST_ASSERT_THROWS_WITH(expr, assert_type, args)                                         \
    do {                                                                                           \
        if(!doctest::getContextOptions()->no_throw) {                                              \
            doctest::detail::ResultBuilder _DOCTEST_RB(doctest::assertType::assert_type, __FILE__, \
                                                       __LINE__, #expr, "", args);                 \
...
#define DOCTEST_WARN_THROWS_WITH(expr, ex) DOCTEST_ASSERT_THROWS_WITH(expr, DT_WARN_THROWS_WITH, ex)
#define DOCTEST_CHECK_THROWS_WITH(expr, ex) DOCTEST_ASSERT_THROWS_WITH(expr, DT_CHECK_THROWS_WITH, ex)
#define DOCTEST_REQUIRE_THROWS_WITH(expr, ex) DOCTEST_ASSERT_THROWS_WITH(expr, DT_REQUIRE_THROWS_WITH, ex)

#define DOCTEST_WARN_THROWS_WITH_AS(expr, message, ex) DOCTEST_ASSERT_THROWS_AS_MESSAGE(expr, ex, DT_WARN_THROWS_WITH_AS, message)
#define DOCTEST_CHECK_THROWS_WITH_AS(expr, message, ex) DOCTEST_ASSERT_THROWS_AS_MESSAGE(expr, ex, DT_CHECK_THROWS_WITH_AS, message)
#define DOCTEST_REQUIRE_THROWS_WITH_AS(expr, message, ex) DOCTEST_ASSERT_THROWS_AS_MESSAGE(expr, ex, DT_REQUIRE_THROWS_WITH_AS, message)
#endif // DOCTEST_CONFIG_WITH_VARIADIC_MACROS
...
#define DOCTEST_REGISTER_EXCEPTION_TRANSLATOR(signature)                                           \
    template <typename DOCTEST_UNUSED_TEMPLATE_TYPE>                                               \
    static inline doctest::String DOCTEST_ANONYMOUS(_DOCTEST_ANON_TRANSLATOR3_)(signature)
...
namespace detail {
    ...
    struct ContextState : ContextOptions, TestRunStats, CurrentTestCaseStats
    {
        ...
        std::set<std::vector<SubcaseSignature> /**/> subcasesPassed;
...
void String::copy(const String& other) {
    if(other.isOnStack()) {
        using namespace std; memcpy(buf, other.buf, len);
    } else {
        setOnHeap();
        data.size     = other.data.size;
        data.capacity = data.size + 1;
        data.ptr      = new char[data.capacity];
        using namespace std; memcpy(data.ptr, other.data.ptr, data.size + 1);
    }
}
...
String::String(const char* in, unsigned in_size) {
    if(in_size <= last) {
        using namespace std; memcpy(buf, in, in_size + 1);
        ...
        data.capacity = data.size + 1;
        data.ptr      = new char[data.capacity];
        using namespace std; memcpy(data.ptr, in, in_size + 1);
    }
}
...
String& String::operator+=(const String& other) {
    ...
    if(isOnStack()) {
        if(total_size < len) {
            ...
            using namespace std; memcpy(buf + my_old_size, other.c_str(), other_size + 1);
            setLast(last - total_size);
        } else {
            // alloc new chunk
            char* temp = new char[total_size + 1];
            // copy current data to new location before writing in the union
            using namespace std; memcpy(temp, buf, my_old_size); // skip the +1 ('\0') for speed
            ...
            using namespace std; memcpy(data.ptr + my_old_size, other.c_str(), other_size + 1);
        }
    } else {
        if(data.capacity > total_size) {
            ...
            using namespace std; memcpy(temp, data.ptr, my_old_size); // skip the +1 ('\0') for speed
            // release old chunk
            delete[] data.ptr;
...
namespace detail {
    ...
    bool TestCase::operator<(const TestCase& other) const {
        if(m_line != other.m_line)
            return m_line < other.m_line;
        using namespace std;
        const int file_cmp = strcmp(m_file, other.m_file);
        if(file_cmp != 0)
            return file_cmp < 0;
...
namespace detail {
    ...
    bool fileOrderComparator(const TestCase* lhs, const TestCase* rhs) {
        ...
        const int res = doctest::detail::stricmp(lhs->m_file, rhs->m_file);
#else  // MSVC
        const int res = strcmp(lhs->m_file, rhs->m_file);
#endif // MSVC
        if(res != 0)
            return res < 0;
...
namespace detail{
    ...
    std::ostream& file_line_to_stream(std::ostream& s, const char* file, int line,
                                      const char* tail = "") {
        const doctest::ContextOptions *const opt = getContextOptions();
...
// =================================================================================================
    ...
    struct ConsoleReporter : public IReporter
    {
        ...
        void test_run_start() DOCTEST_OVERRIDE { printIntro(); }
